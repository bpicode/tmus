...
type AudioSource struct {
	...
	Format FormatType
	// MetadataUpdates reports complete metadata snapshots while Reader is
	// consumed. Empty fields replace previously reported values. The producer
	// owns and closes the channel when Reader is closed or exhausted.
	MetadataUpdates <-chan SourceMetadata
}

// SourceMetadata is a complete snapshot of presentation metadata emitted by an
// audio source. Empty fields replace previously reported values.
