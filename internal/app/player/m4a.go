...
// Adapted for project interfaces.
...
func decodeM4a(rsc io.ReadSeekCloser) (beep.StreamSeekCloser, beep.Format, error) {
	...
	totalLen := int64(r.Duration().Seconds() * float64(sampleRate))
	...
	case m4aCodecAAC:
		...
		if err := decoder.Init(context.Background(), r.CodecConfig()); err != nil {
	...
	case m4aCodecALAC:
		cfg, err := alac.ParseMagicCookie(r.CodecConfig())
		if err != nil {
			return nil, beep.Format{}, err
		}
